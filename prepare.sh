...
if [ "$SKIP_ANNOTATION" = false ]; then
  ...
  docker run -it -v "${PWD}":/a -w /a python:3 bash -c "apt-get update && apt-get --yes install libgeos-dev build-essential cmake libboost-dev libexpat1-dev zlib1g-dev libbz2-dev && pip install -r requirements.txt && python annotate_osm.py"
  echo "done."
  echo
